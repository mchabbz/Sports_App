...
class NFLManager:
    ...
    def get_all_players(team=None, sort_by=None):
        ...
        if sort_by == "player":
            query = query.order_by(NFLPlayer.player.asc())
        elif sort_by == "team":
            query = query.order_by(NFLPlayer.team.asc())
        elif sort_by == "metric_1":
            query = query.order_by(NFLPlayer.metric_1.desc())
        elif sort_by == "metric_2":
            query = query.order_by(NFLPlayer.metric_2.desc())
        elif sort_by == "metric_3":
            query = query.order_by(NFLPlayer.metric_3.desc())
        elif sort_by == "metric_4":
            query = query.order_by(NFLPlayer.metric_4.desc())
        else:
            query = query.order_by(NFLPlayer.team.asc(), NFLPlayer.player.asc())
    ...
    def search_players(query_text=None, search_field="all"):
        ...
        if search_field == "game_id":
            query = query.filter(NFLPlayer.game_id.ilike(search_pattern))
        elif search_field == "league":
            query = query.filter(NFLPlayer.league.ilike(search_pattern))
        elif search_field == "team":
            query = query.filter(NFLPlayer.team.ilike(search_pattern))
        elif search_field == "player":
            query = query.filter(NFLPlayer.player.ilike(search_pattern))
        else:
            query = query.filter(
        ...
        return query.order_by(NFLPlayer.team.asc(), NFLPlayer.player.asc()).all()

    @staticmethod
    def get_player_count():
        return NFLPlayer.query.count()
    ...
    @staticmethod
    def get_all_teams():
        teams = NFLPlayer.query.with_entities(
            NFLPlayer.team
        ).distinct().order_by(
            NFLPlayer.team.asc()
        ).all()
    ...
    def get_player_by_id(player_id):
        ...
        return NFLPlayer.query.get_or_404(player_id)
